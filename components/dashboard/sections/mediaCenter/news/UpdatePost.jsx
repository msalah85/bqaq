            ...
            .then(async (response) => {
                if (postImages?.length > 0 || postVideos?.length > 0 || coverImg) {
                    ...
                    axios.all([...imagesUpload, ...videosUpload, coverImage])
                        .then(axios.spread((...responses) => {
                            console.log(responses.map(response => response.data.model?.id).filter(Boolean));
                            var media = {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} style={{ position: 'relative' }}>
            ...
            <SelectBox
                ...
                placeholder={postDetails.categoryName || "تصنيف الموضوع"}
            ...
            <label htmlFor='dashboard-img-upload' className={"dashboard-edit-img-cont"}>
                ...
                <ReactImageFallback
                    src={coverImg ? URL.createObjectURL(coverImg) : postDetails.coverUrl}
