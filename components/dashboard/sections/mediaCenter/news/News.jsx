...
export default function NewsDashboard() {
    ...
    const fetchTableData = () => {
        ...
        requester.get("/content/pages/media-center", {}).then((response) => {
            setDataStatus(null);
            setRecords(response.data.model?.containers?.news);
        }).catch(() => {
            setRecords([]);
    ...
    return (
        <div className='ourProjectsBody'>
            ...
            {
                caseToShow == 'add' ? <AddPost fetchTableData={fetchTableData} setCaseToShow={setCaseToShow} postType={1} />
                    :
                    caseToShow == 'update' ? <UpdatePost fetchTableData={fetchTableData} setCaseToShow={setCaseToShow} selectedPost={selectedPost} postType={1} />
                        :
