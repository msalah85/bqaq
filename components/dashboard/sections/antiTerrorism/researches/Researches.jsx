...
export default function ResearchesDashboard() {
    ...
    const fetchTableData = () => {
        runLoadPanel();
        requester.get("/terrorism/research/listing").then((response) => {
            setDataStatus(null);
    ...
    const onRowInserted = (e) => {
        ...
        requester.post('/terrorism/research/create-new-article', { ...e.data }).then((res) => {
            Success();
            fetchTableData();
        }).catch(() => {
            Failed();
    ...
    const onRowUpdated = (e) => {
        runLoadPanel();
        requester.put('/terrorism/research/update-article', { ...e.data }).then(() => {
            Success();
            fetchTableData();
        }).catch(() => {
            Failed();
    ...
    const onRowRemoved = (e) => {
        runLoadPanel();
        requester.delete(`/terrorism/research/delete-article?articleId=${e.data.id}`).then(() => {
            Success();
            fetchTableData();
    ...
    return (
        <div className='whoWeAreBody'>
            <div className='section-title'>بحوث تأصيلية</div>
