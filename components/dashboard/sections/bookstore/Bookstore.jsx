import React, { useEffect, useState } from 'react'
import DataGrid, {
    Column,
    Editing,
    Popup,
    Form,
    Pager,
    Paging,
    Grouping,
    GroupPanel,
    SearchPanel,
    HeaderFilter,
    RequiredRule,
    Lookup,

} from 'devextreme-react/data-grid';
import 'devextreme-react/text-area';

import { Item } from 'devextreme-react/form';

import { Failed, Success } from "../../alertPopup/AlertPopup";
import requester from '../../../../requester/requester';
import Spinner from '../../../../reusable/spinner/Spinner';
import ReactImageFallback from 'react-image-fallback';
import axios from 'axios';
import { FileSizeValidator } from '../../../../utils/FileSizeValidator';

export default function BookstoreDashboard() {

    const [records, setRecords] = useState([]);
    const [languages, setLanguages] = useState('loading');
    const [showLoadPanel, setShowLoadPanel] = useState(false);
    const [dataStatus, setDataStatus] = useState(null);
    var newImage = null;
    var bookFile = null;

    useEffect(() => {
        setShowLoadPanel(false);
    }, [records]);

    useEffect(() => {
        fetchLanguages();
        fetchTableData();
    }, []);

    const runLoadPanel = () => {
        setShowLoadPanel(true)
    }

    const fetchTableData = () => {
        runLoadPanel();
        requester.get("/books/get")
            .then((response) => {
                setDataStatus(null);
                setRecords(response.data.model);
            }).catch(() => {
                setRecords([]);
                setDataStatus('فشل الاتصال')
            })
    }

    const fetchLanguages = () => {
        requester.get("/lessons/languages/get")
            .then((response) => {
                setLanguages(response.data.model);
            }).catch(() => {
                setLanguages([]);
            })

    }

    const onRowInserted = (e) => {
        runLoadPanel();
        console.log('new', e, newImage);
        requester.post('/books/create', { ...e.data, languageId: e.data.languageName?.id })
            .then((res) => {
                if (newImage || bookFile) {
                    let fileContainer = [];

                    if (newImage) {
                        let image = new FormData();
                        image.append("Cover", newImage, newImage.name);
                        image.append("Id", res.data?.model?.id);
                        fileContainer.push(
                            requester.patch('/books/upload-cover', image, { headers: { "Content-Type": "multipart/form-data" } })
                        )
                    }

                    if (bookFile) {
                        let bookPdf = new FormData();
                        bookPdf.append("Document", bookFile, bookFile.name);
                        bookPdf.append("Id", res.data?.model?.id);
                        fileContainer.push(
                            requester.patch('/books/upload-document', bookPdf, { headers: { "Content-Type": "multipart/form-data" } })
                        )
                    }
                    axios.all([...fileContainer]).then(axios.spread((...responses) => {
                        newImage = null;
                        bookFile = null
                        Success();
                        fetchTableData();
                    })).catch(() => {
                        newImage = null;
                        bookFile = null
                        Success();
                        fetchTableData();
                    })


                } else {
                    Success();
                    fetchTableData();
                }

            }).catch(() => {
                Failed();
                fetchTableData();
            })

    }

    const onRowUpdated = (e) => {
        runLoadPanel();

        requester.put('/books/update', { ...e.data, languageId: e.data.languageName?.id || e.data.languageId })
            .then((res) => {
                if (newImage || bookFile) {
                    let fileContainer = [];

                    if (newImage) {
                        let image = new FormData();
                        image.append("Cover", newImage, newImage.name);
                        image.append("Id", res.data?.model?.id);
                        fileContainer.push(
                            requester.patch('/books/upload-cover', image, { headers: { "Content-Type": "multipart/form-data" } })
                        )
                    }

                    if (bookFile) {
                        let bookPdf = new FormData();
                        bookPdf.append("Document", bookFile, bookFile.name);
                        bookPdf.append("Id", res.data?.model?.id);
                        fileContainer.push(
                            requester.patch('/books/upload-document', bookPdf, { headers: { "Content-Type": "multipart/form-data" } })
                        )
                    }

                    axios.all([...fileContainer]).then(axios.spread((...responses) => {
                        newImage = null;
                        bookFile = null
                        Success();
                        fetchTableData();
                    })).catch(() => {
                        newImage = null;
                        bookFile = null
                        Failed();
                        fetchTableData();
                    })

                } else {
                    Success();
                    fetchTableData();
                }
            }).catch(() => {

                Failed();
                fetchTableData();
            })
    }

    const onRowRemoved = (e) => {
        runLoadPanel();
        requester.delete(`/books/delete?bookId=${e.data.id}`).then(() => {
            Success();
            fetchTableData();
        }).catch(() => { Failed(); fetchTableData(); })
    }

    const ImageView = (rowData) => {
        return <div className={"dashboard-img-cont"}>
            <ReactImageFallback
                src={rowData.data?.coverUrl}
                fallbackImage={'/assets/fallbacks/Admin Avatar.svg'}
                alt={rowData.data?.name}
                className="dashboard-img"
            />
        </div>
    }

    const UpdateImage = (e) => {
        try {
            const onImageChange = (event) => {
                console.log('event', event);
                if (event.target.files[0]) {
                    let imgSrc = URL.createObjectURL(event.target.files[0]);
                    document.getElementById("dashboard-img-update") && (document.getElementById("dashboard-img-update").src = imgSrc)
                    newImage = event.target.files[0];
                    e.data.setValue(imgSrc)
                }
            }
            return <>
                <label htmlFor='dashboard-img-upload' className={"dashboard-edit-img-cont"}>
                    <div className={"dashboard-img-cont-overlay"} >اضف</div>
                    <ReactImageFallback
                        src={e.data?.value}
                        fallbackImage={'/assets/fallbacks/Admin Avatar.svg'}
                        className="dashboard-img"
                        id={"dashboard-img-update"}
                    />
                </label>
                <form hidden>
                    <input id={"dashboard-img-upload"}
                        type="file"
                        accept="image/*"
                        onChange={(e) => { FileSizeValidator(e, onImageChange) }}
                    />
                </form></>
        }
        catch {
            console.log('error');
        }
    }

    const UpdateFile = (e) => {
        try {

            const onFileChange = (event) => {
                if (event.target.files[0]) {
                    bookFile = event.target.files[0];
                    e.data.setValue(bookFile);
                }
            }

            return <div>
                <label htmlFor='input-file-upload' style={{ display: 'flex', justifyContent: 'center' }}>
                    <div className='add-new-file' style={{ marginTop: '3rem' }}>اضافة ملف للكتاب</div>
                </label>
                <input
                    id={"input-file-upload"}
                    type="file"
                    onChange={(e) => { FileSizeValidator(e, onFileChange) }}
                    hidden
                    accept="application/msword, application/vnd.ms-excel, application/vnd.ms-powerpoint,text/plain, application/pdf"
                />
            </div>
        }
        catch {
            console.log('error');
        }
    }

    const onEditorPreparing = (e) => {
        console.log(e);
        if (e.dataField === "languageName" && e.parentType === "dataRow") {
            e.editorOptions.placeholder = "اختر لغة الكتاب";
        }
    }

    return (
        <div className='whoWeAreBody'>
            <div className='section-title'>المكتبه</div>

            <div className='table-container' style={{ width: '100%', position: 'relative' }}>
                {(showLoadPanel || languages == 'loading') && <Spinner />}
                <DataGrid
                    noDataText={dataStatus || 'لا توجد بيانات'}
                    dataSource={records}
                    allowColumnReordering={true}
                    allowColumnResizing={true}
                    showBorders={true}
                    showRowLines={true}
                    cellHintEnabled={true}
                    wordWrapEnabled={true}
                    rowAlternationEnabled={true}
                    rtlEnabled
                    onRowUpdated={onRowUpdated}
                    onRowInserted={onRowInserted}
                    onRowRemoved={onRowRemoved}
                    width='100%'
                    onEditorPreparing={onEditorPreparing}
                >

                    <HeaderFilter visible={true} />
                    <GroupPanel visible={true} emptyPanelText='اسحب عنوان لهنا' />
                    <SearchPanel visible={true} all placeholder='بحث...' />
                    <Grouping autoExpandAll={true} />
                    <Paging defaultPageSize={10} />
                    <Pager
                        showPageSizeSelector={true}
                        allowedPageSizes={[5, 10, 20]}
                        showInfo={true}
                        showNavigationButtons={true}
                    />

                    <Editing
                        mode="popup"
                        allowUpdating
                        allowDeleting
                        allowAdding
                        useIcons={true}
                        texts={{
                            confirmDeleteMessage: 'هل انت متاكد انك تريد مسح هذا النص؟',
                            saveRowChanges: "حفظ",
                            cancelRowChanges: "إلغاء",
                            deleteRow: "مسح",
                            editRow: "تعديل",
                            addRow: 'اضف جديد',

                        }}
                    >
                        <Popup title="اضف قسم جديد" showTitle={true} width={900} height={800} maxHeight={'80%'} rtlEnabled />

                        <Form labelLocation={"top"} >

                            <Item dataField="bookName">
                                <RequiredRule message="الاسم مطلوب" />
                            </Item>

                            <Item dataField="languageName">
                                <RequiredRule message="اللغه مطلوبه مطلوب" />
                            </Item>

                            <Item dataField="content" editorType="dxTextArea" colSpan={2} editorOptions={{ height: 200 }} >
                                <RequiredRule message="الوصف مطلوب" />
                            </Item>
                            <Item dataField="author">
                                <RequiredRule message="المؤلف مطلوب" />
                            </Item>

                            <Item dataField="price">
                                <RequiredRule message=" السعر مطلوب" />
                            </Item>

                            <Item colSpan={2} dataField="documentUrl" />

                            <Item dataField="coverUrl" />

                            <Item dataField="fileUpload" />

                        </Form>

                    </Editing>

                    <Column
                        dataField="coverUrl"
                        caption='غلاف الكتاب'
                        alignment={"center"}
                        width={'100'}
                        allowSorting={false}
                        cellRender={ImageView}
                        editCellComponent={UpdateImage}
                    />

                    <Column dataField="bookName" alignment={"center"} caption='عنوان الكتاب' />

                    <Column dataField="languageName" caption='لغة الكتاب' alignment="center">
                        <Lookup dataSource={languages} displayExpr="name" />
                    </Column>

                    <Column dataField="content" alignment={"center"} caption='محتوي الكتاب' />

                    <Column dataField="author" alignment={"center"} caption='مؤلف الكتاب' />

                    <Column dataField="price" alignment={"center"} caption='سعر الكتاب' />

                    <Column dataField="documentUrl" caption='رابط الكتاب' visible={false} />

                    <Column dataField="fileUpload" caption='ملف للكتاب' visible={false} editCellComponent={UpdateFile} />

                </DataGrid>
            </div >

        </div >
    )
}
