...
export default function TagsDashboard() {
    ...
    const onRowInserted = (e) => {
        ...
        requester.post('/lessons/tags/create', { name: e.data.name })
            .then((res) => {
    ...
    const onRowUpdated = (e) => {
        runLoadPanel();
        requester.put('/lessons/tags/update', { ...e.data, isDeleted: undefined })
            .then((res) => {
                Success();
                fetchTableData();
            }).catch(() => {
    ...
    const toggleActivation = (id) => {
        runLoadPanel();
        requester.put('/lessons/tags/delete-or-restore?tagId=' + id).then(() => {
            Success();
            fetchTableData();
        }).catch(() => {
            Failed();
    ...
    return (
        <div className='whoWeAreBody'>
            <div className='section-title'>التصنيف</div>
